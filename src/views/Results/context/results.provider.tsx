    ...
    try {
      ...
      if (_get(response, "data.jobs").length) {
        ...
        dispatch({
          type: "UPDATE_VALUE",
          key: "totalPages",
          value: Math.abs(_get(response, "data.jobs", []).length / perPage)
        });
      } else {
        dispatch({
          type: "UPDATE_VALUE",
          key: "errorMessage",
          value: _get(response, "data.Error.Error")
        });
        dispatch({ type: "UPDATE_VALUE", key: "results", value: [] });
      }
      ...
      value={
        {
          ...
          incrementPage: () => {
            if (Math.abs((currentPageIndex as any) + 1) > totalPages) {
              return;
            }
            ...
            dispatch({
              ...
              value: Math.abs((currentPageIndex as any) + 1)
            });
            updatePaginatedResults();
          },
          decrementPage: () => {
          ...
          setKeyword: (keywordString: string) =>
            dispatch({
              type: "UPDATE_VALUE",
              key: "keyword",
              value: keywordString
