...
const SearchForm: React.FC<IProps> = ({ submitHandler, cssClasses }) => {
  ...
  return (
    <form
      className={`flex justify-center w-full md:max-w-md relative mt-4 sm:mt-0 ${cssClasses}`}
      onSubmit={e => {
        submitHandler ? handleSubmit(e) : e.preventDefault();
      }}
    >
      <input
        type="search"
      ...
      {submitHandler && (
        <Link
          to={`/results?keyword=${localKeyword}`}
